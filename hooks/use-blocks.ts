...
export function useBlocks(initialBlocks: Block[] = [], sourceKey?: string) {
  ...
  const updateBlock = useCallback((id: string, content: any) => {
    if (!id) {
      console.warn("[notion] Attempted to update block with undefined ID");
      return;
    }
  ...
  const deleteBlock = useCallback((id: string) => {
    if (!id) {
      console.warn("[notion] Attempted to delete block with undefined ID");
      return;
    }
  ...
  const duplicateBlock = useCallback(
    (id: string) => {
      if (!id) {
        console.warn("[notion] Attempted to duplicate block with undefined ID");
        return;
      }
      ...
      setBlocks((prev) => {
        ...
        if (!block) {
          console.warn("[notion] Block not found for duplication:", id);
          return prev;
        }
