...
export function BlockWrapper({ block, children, onDelete, onDuplicate, onAddBlock, index }: BlockWrapperProps) {
  const [isHovered, setIsHovered] = useState(false)
  const [isMenuOpen, setIsMenuOpen] = useState(false)

  if (!block || !block.id) {
    console.error("[v0] BlockWrapper received invalid block:", block)
  ...
  const handleDeleteBlock = () => {
    if (!block.id) {
      console.error("[v0] Delete button clicked with undefined block ID")
      return
    }
    console.log("[v0] Delete button clicked for block:", block.id)
    onDelete(block.id)
  }

  const handleDuplicateBlock = () => {
    if (!block.id) {
      console.error("[v0] Duplicate button clicked with undefined block ID")
      return
    }
    console.log("[v0] Duplicate button clicked for block:", block.id)
    onDuplicate(block.id)
  }

  const handleAddBlock = (blockType: Block["type"]) => {
    console.log("[v0] Add block button clicked, type:", blockType, "after index:", index)
          ...
          <div className="flex items-center gap-1 py-2 pr-2">
            ...
            <DropdownMenu open={isMenuOpen} onOpenChange={(open) => {
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                <div className="px-2 py-1.5 text-sm font-medium text-muted-foreground">Add Block</div>
                <DropdownMenuSeparator />
                ...
                <div className="px-2 py-1.5 text-sm font-medium text-muted-foreground">Actions</div>
                <DropdownMenuItem onClick={handleDuplicateBlock} className="flex items-center gap-2 cursor-pointer">
